...
#        if self.money.score >= Box.cost_box:
        for item in self.boxs:
            if (mouseX > item.x + item.w/2 and mouseX < item.x + item.w/2*3 and 
                ...
                mouseY > item.y + item.h/2 and mouseY < item.y + item.h/2*3 and item.y + item.h/2 < height/2+20 + item.h/2):
                #self.money.score -= Box.cost_box
                self.boxs.append(Box(item.x, item.y + item.h, 35, 35, 0, 0, 0))
    ...
    def outline(self):
        for item in self.boxs:
            if (mouseX > item.x + item.w/2 and mouseX < item.x + item.w/2*3 and 
                ...
                mouseY > item.y + item.h/2 and mouseY < item.y + item.h/2*3):
                noFill()
                rect(item.x, item.y + item.h, 35, 35)

    def touch_to_box(self):
        for item in self.boxs:
            if abs(self.x-self.w/2 - (item.x + item.w/2)) < self.speedx and item.y - item.h/2 < self.y + self.h/2 and item.y + item.h/2 > self.y - self.h/2 and self.x > item.x:
                self.x = item.x + item.w/2 + self.w/2 + 0.1
            elif abs(self.x+self.w/2 - (item.x - item.w/2)) < self.speedx and item.y - item.h/2 < self.y + self.h/2 and item.y + item.h/2 > self.y - self.h/2 and self.x < item.x:
                self.x = item.x - item.w/2 - self.w/2 - 0.1
            ...
            if item.x - item.w/2 < self.x + self.w/2 and self.x - self.w/2 < item.x + item.w/2 and abs((self.y + self.h/2) - (item.y - item.h/2)) < self.speedy:
                ...
                self.speedy = 0
            elif item.x - item.w/2 < self.x + self.w/2 and self.x - self.w/2 < item.x + item.w/2 and abs((self.y - self.h/2) - (item.y + item.h/2)) < abs(self.speedy):
                self.y = item.y + item.h/2 + self.h/2 + 0.2
